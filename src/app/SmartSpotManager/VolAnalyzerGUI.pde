...
public class VolAnalyzerGUI
{
  ...
  public VolAnalyzerGUI(PApplet par, ControlP5 cp, String pname, float x, float y, SpotVirtual sv)
  {
    parent = par;
    cp5 = cp;
    pannel_name = pname;
  ...
  void handleChange(float value, int id)
  {
    ...
    {
    case 0:
      for (SpotVirtual sv : spots) sv.setDt(int(value));
      break;
    case 1:
      for (SpotVirtual sv : spots) sv.setWindow(byte(value));
      break;
    case 2:
      for (SpotVirtual sv : spots) sv.setTrsh(int(value));
      break;

    case 3:
      for (SpotVirtual sv : spots) sv.setVolDt(byte(value));
      break;
    case 4:
      for (SpotVirtual sv : spots) sv.setVolK(byte(value));
      break;
    case 5:
      for (SpotVirtual sv : spots) sv.setVolMin(byte(value));
      break;
    case 6:
      for (SpotVirtual sv : spots) sv.setVolMax(byte(value));
      break;

    case 7:
      for (SpotVirtual sv : spots) sv.setPulseMax(byte(value));
      break;
    case 8:
      for (SpotVirtual sv : spots) sv.setPulseMin(byte(value));
      break;
    case 9:
      for (SpotVirtual sv : spots) sv.setPulseTimeout(int(value));
      break;
    }
  }
