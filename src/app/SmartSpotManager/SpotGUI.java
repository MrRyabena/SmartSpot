...
public class SpotGUI
{
  ...
  public SpotGUI(PApplet parrent, float x, float y, SpotVirtual spot, String pannel_name)
  {
    ...
    this.cp5 = new ControlP5(parrent);
  ...
  public void updateSpot()
  {
    ...
    // set bright
    val = int(cp5.get(Knob.class, pannel_name + "bright").getValue());
    ...
    // set fade
    val = int(cp5.get(Toggle.class, pannel_name + "fade").getValue());
    spot_virtual.setFadeMode(boolean(val));

    val = int(cp5.get(Slider.class, pannel_name + "fd_time").getValue());
    spot_virtual.setFadePeriod(val);
  ...
  void hide()
  {
    ...
    {
      if (c.getName().startsWith(pannel_name)) c.hide();
    }
  }
  ...
  public void setEnabled(boolean flag)
  {
    ...
    {
      if (c.getName().startsWith(pannel_name)) cp5.getController(c.getName()).setLock(flag); //.setOpen(flag);
      // cp5
  ...
  {
    spot_virtual.setMusicEffect(effect);
    if (syncing_flag) for (SpotGUI x : syncing) x.setMusicEffect(effect);
  }
  ...
  int getBright()
  {
    return int(cp5.get(Knob.class, pannel_name + "bright").getValue());
  }
  ...
  void handle_ef_fire_toggle(CallbackEvent event)
  {
    int val = int(event.getController().getValue());
    if (val == 1) {
      cp5.get(Toggle.class, pannel_name + "fire").setColorActive(color(#00ff00));
      flag_fr = true;
    } else {
      cp5.get(Toggle.class, pannel_name + "fire").setColorActive(color(#ff0000));
      flag_fr = false;
    }
  }
  ...
  void NewYear_tick()
  {
    if (state_ny != flag_ny)
    {
      if (!state_ny && flag_ny)
      {
        ...
        cp5.get(Toggle.class, pannel_name + "fade").setValue(true);
        cp5.get(Slider.class, pannel_name + "fd_time").setValue(1500);
      }
      state_ny = flag_ny;
    }
  ...
  void guiSetup() {
    ...
    ;
    cp5
      .addSlider(pannel_name + "fd_time_2")
      .setCaptionLabel("fade T")
      ...
      .onClick(new CallbackListener() {
      public void controlEvent(CallbackEvent event) {
        cp5.get(Slider.class, pannel_name + "fd_time").setValue(event.getController().getValue());
      }
    }
    )
    ;
    ...
    cp5
      ...
      public void controlEvent(CallbackEvent event) {
        int val = int(event.getController().getValue());
        if (val == 1) {
          cp5.get(Toggle.class, pannel_name + "NewYear").setColorActive(color(#00ff00));
          flag_ny = true;
        } else {
          cp5.get(Toggle.class, pannel_name + "NewYear").setColorActive(color(#ff0000));
          flag_ny = false;
        }
      }
    }
  ...
  void addColors() {
    ...
    cp5
      .addDropdownList(pannel_name + "RGBmusic")
