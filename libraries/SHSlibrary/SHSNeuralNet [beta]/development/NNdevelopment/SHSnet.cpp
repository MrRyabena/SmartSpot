...
net::net(const std::vector<uint8_t> &topology)
{
	uint8_t numLayers = topology.size();
	for (uint8_t layerNum = 0; layerNum < numLayers; ++layerNum) {
		layers.push_back(Layer());
		uint8_t numOutputs = layerNum == topology.size() - 1 ? 0 : topology[layerNum + 1];


		for (uint8_t neuronNum = 0; neuronNum <= topology[layerNum]; ++neuronNum) {
